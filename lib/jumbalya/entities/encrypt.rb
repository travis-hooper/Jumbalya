...
module Jumbalya
  ...
  def self.encrypt(string, password)
    if password.length < 5
      return "Your password is too short. It must be 5 charaters or longer"
    end
    set_pass(password)
    ...
    for i in 0...nums.length # takes each array element and assigns 2 letter pair for each element
      y = (nums[i].to_i * @counter) + @counter3 + @digested_pass[i%39]
      if y > ymax
        ymax = y
      end
  ...
  def self.unencrypt(string, password)
    ...
    for i in 0...letters.length
      y = ((letters[i] - @counter3 - @digested_pass[i%39])/@counter).to_s
      if y.length == 2
        nums << y
      else
        nums << ('0' + y)
  ...
  @@hexadecimal_to_decimal = {'a'=>11, 'b'=>12, 'c'=>13, 'd'=>14, 'e'=>15, 'f'=>16}
  ...
  def self.set_pass(password)
    @digested_pass = Digest::SHA1.hexdigest(password + '|\\|aC1isSALT').split('').map { |x| @@hexadecimal_to_decimal[x] ? @@hexadecimal_to_decimal[x] : x.to_i  }
  end

  def self.set_counter
    @counter = @digested_pass[4] % 7
    @counter2 = @digested_pass[@counter + 20] % 4
